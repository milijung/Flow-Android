...
class ListDetailActivity : AppCompatActivity() {
    ...
    fun InsertData(context: Context) {
        ...
        AppDatabase.getCategoryInstance(context)
            ?.CategoryDao()?.insert(Category("자기계발",R.drawable.ic_category_selfimprovement,1,6,false))
        AppDatabase.getCategoryInstance(context)
        ...
        AppDatabase.getCategoryInstance(context)
            ?.CategoryDao()?.insert(Category("오락·취미",R.drawable.ic_category_entertainment,1,11,false))
        AppDatabase.getCategoryInstance(context)
        ...
        AppDatabase.getListInstance(context)
            ?.ListDao()?.insert(List(1,"2023","01","15","03:47","(주)우아한형제들",10000,"",1,true))
    }
